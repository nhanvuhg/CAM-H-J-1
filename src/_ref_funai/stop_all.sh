...
# Kill camera nodes (Graceful first to avoid Kernel Deadlock!)
echo "  Stopping camera nodes gracefully..."
pkill -TERM -f "csi_dual_camera_node" 2>/dev/null
pkill -TERM -f "csi_camera_node" 2>/dev/null
pkill -TERM -f "rpicam-vid" 2>/dev/null
pkill -TERM -f "yolo_ros_hailort_cpp" 2>/dev/null
pkill -TERM -f "system_csi_dual_model" 2>/dev/null
...
pkill -KILL -f "csi_camera_node" 2>/dev/null
pkill -KILL -f "rpicam-vid" 2>/dev/null

# Verify all stopped
REMAINING=$(ps aux | grep -E "(robot_logic_node|dobot_bringup|gripper_festo|csi_|rpicam-vid|yolo|bbox_drawer|snap7|ros2_qml_gui1)" | grep -v grep | wc -l)
...
if [ $REMAINING -eq 0 ]; then
    ...
    ps aux | grep -E "(robot_logic_node|dobot_bringup|gripper_festo|csi_|rpicam-vid|yolo)" | grep -v grep
    echo ""
    echo "Try running: sudo killall -9 robot_logic_node dobot_bringup gripper_festo_node.py csi_dual_camera_node rpicam-vid"
fi
